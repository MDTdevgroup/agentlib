...
function _convertInput(input) {
    ...
    // First pass to map call IDs to names and signatures for tool responses
    ...
    for (const object of input) {
        ...
        if (object.role === 'system') {
            ...
            const sig = object.thoughtSignature || object.thought_signature || object.signature || (callId ? callIdToSignature.get(callId) : undefined);
            if (sig) {
            ...
            if (Array.isArray(outputResult)) {
                const isSubcontentArray = outputResult.length > 0 && outputResult.every(p => p && typeof p === 'object' && (p.type === 'text' || p.type === 'image'));
                if (!isSubcontentArray) {
                    outputResult = { result: outputResult };
                }
            } else if (typeof outputResult === 'number' || typeof outputResult === 'boolean' || outputResult === null || outputResult === undefined) {
            ...
            const textStr = typeof thoughtText === 'string' ? thoughtText : JSON.stringify(thoughtText);
...
export async function chat(client, input, { model = defaultModel, inputSchema, outputSchema, tools, signal, previousInteractionId, ...options } = {}) {
    if (!client?.interactions || typeof client.interactions.create !== 'function') {
        throw new Error("Gemini Interactions API requires @google/genai >= 2.20.0 with client.interactions support.");
    }
